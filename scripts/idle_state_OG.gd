...
func do_something(delta):
	...
	if(parent_enemy.commands[current_command].type == "move" and max_command != 0):
		...
		#suruh kejar
		#parent_enemy.current_state.next_state[0]
		#print("NEXT COMMAND ", next_state[0])
		...
		#Nev
		if(parent_enemy.global_position - parent_enemy.movement_target_position).length() <= 0.05:
	...
	#
		...
		##rotation & delay
		#is_rotating = true
		#await parent_enemy.look_towards(parent_enemy.commands[current_command].target_node.global_position)
		#await get_tree().create_timer(2.0).timeout
		#
